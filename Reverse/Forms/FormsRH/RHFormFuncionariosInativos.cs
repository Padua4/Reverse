...
namespace Reverse
{
    public partial class FormFuncionariosInativos : Form
    {
        ...
        public FormFuncionariosInativos(int _usuarioId)
        {
            InitializeComponent();
            ConfigurarControles();
            CarregarLista();
            LimparCampos(); // inicia em modo inserção
        }
        ...
        private void CarregarLista()
        {
            ...
            using (var da = new SqlDataAdapter(
                @"SELECT FuncionarioID, Nome, Empresa, Funcao, Status, MotivoInativacaoTemp
          FROM Funcionarios
        ...
        private void ForcarCorLabels(Control container, Color cor)
        {
        ...
        private void CarregarFuncionario(int id)
        {
            ...
            using (var cmd = new SqlCommand("SELECT * FROM Funcionarios WHERE FuncionarioID = @id", con))
            {
                ...
                using (var dr = cmd.ExecuteReader())
                {
                    ...
                    txtNome.Text = dr["Nome"]?.ToString() ?? string.Empty;
                    dtpNascimento.Value = ToDate(dr["DataNascimento"], DateTime.Today);
                    txtRG.Text = dr["RG"]?.ToString() ?? string.Empty;
                    txtCPF.Text = dr["CPF"]?.ToString() ?? string.Empty;
                    txtEmail.Text = dr["Email"]?.ToString() ?? string.Empty;

                    txtCNH.Text = dr["CNH"]?.ToString() ?? string.Empty;
                    ...
                    txtASO.Text = dr["ASO"]?.ToString() ?? string.Empty;

                    txtEnderecoRua.Text = dr["EnderecoRua"]?.ToString() ?? string.Empty;
                    txtEnderecoBairro.Text = dr["EnderecoBairro"]?.ToString() ?? string.Empty;
                    txtEnderecoNumero.Text = dr["EnderecoNumero"]?.ToString() ?? string.Empty;
                    txtEnderecoCEP.Text = dr["EnderecoCEP"]?.ToString() ?? string.Empty;
                    ...
                    BloquearCampos(pnlInfo);

                    // Melhor tratamento da exibição do motivo
                    string motivoInativacao = dr["MotivoInativacaoTemp"]?.ToString() ?? "Não informado";
                    MessageBox.Show(
                        $"Motivo da inativação: {motivoInativacao}",
                        "Informação",
        ...
        private void BloquearCampos(Control container)
        {
            foreach (Control ctrl in container.Controls)
            {
                if (ctrl is Label lbl)
                {
                    lbl.Enabled = true; // volta a ser "ativa" visualmente
